 ...
 <%@include file="../inc/left.jsp"%>
		...
		<form name="flist" class="local_sch01 local_sch">
			...
			<label for="sfl" class="sound_only">검색대상</label> <select name="sfl"
				id="sfl">
				<option value="num" selected="selected">환불번호</option>
		...
		<div class="formTitle">
			환불신청목록 <span>검색조건에 해당하는 리스트를 확인 할수 있습니다.</span>
		</div>
		...
		<form name="signForm" method="post" onsubmit="return false;">
			...
			<table class="listTbl fixed">
				<tbody>
					<tr height="25" align="center" bgcolor="#5e718f">
						<th class="listTitle">환불번호</th>
						<th class="listTitle">주문번호</th>
						<th class="listTitle">상품명</th>
						<th class="listTitle">주문수량</th>
						<th class="listTitle">신청날짜</th>
						<th class="listTitle">신청자</th>
					...
					<c:forEach items="${aOrderList}" var="aOrderList">
						...
						<tr>
							...
							<td class="listData"><c:out value="${aOrderList.orderDate}" /></td>
							<td class="listData"><nobr>
									<b><a href="/"><c:out
												value="${aOrderList.memId} / ${aOrderList.memNm}" /></a></b>
								</nobr></td>
			...
			<c:forEach var="i" begin="${pageDTO.startPage }"
				...
				<a
					href="${pageContext.request.contextPath}/aOrder/aOrderList?pageNum=${i}">
					〔${i}〕 </a>
			</c:forEach>
